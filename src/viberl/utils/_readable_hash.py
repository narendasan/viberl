...
def generate_phrase_hash(num: Any) -> str:
    ...
    phrase_components = [
        ...
        verbs,
        adverbs,
    ]  # Structure of the phrase: Adjective Noun Verb Adverb
    phrase_words = []
    current_number = abs(
        number
    )  # Use absolute value for word selection, handle negative later

    for word_list in phrase_components:
        list_length = len(word_list)
        if (
            list_length == 0
        ):  # Handle empty word lists gracefully (though they shouldn't be empty)
