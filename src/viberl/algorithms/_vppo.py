...
def calculate_gae(
    ...
    def _get_advantages(
        advantage_and_next_value: Tuple[chex.Array, chex.Array],
        transition: Trajectory
    ) -> Tuple[Tuple[chex.Array, chex.Array], chex.Array]:
        # transition: Slice of a Trajectory.

        advantage, next_value = advantage_and_next_value
        delta = (
            transition.reward.squeeze()  # For gymnax envs that return shape (1, )
            + gamma * next_value * (1 - transition.done.astype(jnp.int32))
            - transition.value
        ) #type: ignore
        advantage = (
            delta + gamma * gae_lambda * (1 - transition.done.astype(jnp.int32)) * advantage
        )
...
def calculate_dcd_gradients(agents: VecAgent, normalize_obs: bool, normalize_returns: bool):
    pass

class VPPO(OnPolicyMixin, NormalizeObservationsMixin, NormalizeRewardsMixin, Algorithm):
    ...
    def train_iteration(self, ts):
        ...
        advantages, targets = calculate_gae(trajectories, last_val, self.gamma, self.gae_lambda)
